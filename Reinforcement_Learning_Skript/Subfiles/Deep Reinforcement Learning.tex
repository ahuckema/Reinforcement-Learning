...
\begin{align*}
    ...
    &\stackrel{(**)}{\approx} \sum_{t=0}^{T-1} \gamma^t \mathbb{E}^{\pi_{\theta^{old}}} [ \frac{\nabla_{\theta} \pi_\theta(A_t;S_t) }{\pi_{\theta^{old}}} \textbf{1}_{\left|  \frac{\nabla_{\theta} \pi_\theta(A_t;S_t) }{\pi_{\theta^{old}} (A_t;S_t)} -1 \right| \leq \frac{1}{\epsilon}}  A_t^{\pi_{\theta^{old}}} (S_t,A_t)  ]\\
    &= T \mathbb{E}^{U \sim \mathcal{U}(\{0,...,T-1\})} \left[  \gamma^U \mathbb{E}^{\pi_{\theta^{old}}} [ \frac{\nabla_{\theta} \pi_\theta(A_U;S_U) }{\pi_{\theta^{old}}} \textbf{1}_{\left|  \frac{\nabla_{\theta} \pi_\theta(A_t;S_t) }{\pi_{\theta^{old}} (A_t;S_t)} -1  \right| \leq \frac{1}{\epsilon}}  A_U^{\pi_{\theta^{old}}} (S_U,A_U)  ] \right]\\
...
is unbiased. 
\rmkb{
    The GAE (General advantage estimator) combines all $k$-step estimators using exponentially decaying weights controlled by $\lambda \in [0,1]$: 
    \[
    ...
    Note that this estimator assume access to all $k$-step estimators, i.e. access to infinite trajectories. In practice, trajectories are never infinite. So it is reasonable to only consider a finite time horizon.
}
\rmkb{
